...
import { moduleTutorials } from "../../../../model/moduleNotes/moduleTutorials";
...
import { QuickDocCellController } from "../quickDoc/quickDockCell";
import { QuickDocSectionCell } from "../quickDoc/quickSectionCell";
import { ModuleTutorialCellController } from "./moduleTutorialCellController";


export class ModuleTutorialsController extends View implements UICollectionViewDatasource, UICollectionViewDelegate,NotesSearchBarInterface {
    ...
    cvNumberOfItemsInSection(cv: UICollectionView, section: number): number {
        return moduleTutorials[section].modules.length;
    }
    cvNumberOfSections(cv: UICollectionView): number {
        return moduleTutorials.length
    }
    cvCellForItemAt(cv: UICollectionView, indexPath: IndexPath): UICollectionViewCell {
        let cell = new ModuleTutorialCellController(undefined,moduleTutorials[indexPath.section].modules[indexPath.item])
        return cell
    }
    ...
    cvCellForSectionAt(cv: UICollectionView, sectionCellType: UICollectionViewSectionCellType, section: number): UICollectionVievSectionCell | undefined {
        if (sectionCellType == UICollectionViewSectionCellType.header) {
            let mSection = new QuickDocSectionCell(moduleTutorials[section].sectionName ?? "fgdsfsd",leftMenuViewerModuleSectionView);
            return mSection
        } 
        return undefined;
    }
